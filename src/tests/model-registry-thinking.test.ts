...
test("model-registry: handles -thinking suffix (upstream a63f054)", () => {
  ...
  const tokenDivisor = getModelTokenDivisor("qwen3.7-plus-thinking");
  assert.equal(tokenDivisor, 2.0);
});
...
test("model-registry: handles -no-thinking suffix", () => {
  ...
  const tokenDivisor = getModelTokenDivisor("qwen3.7-plus-no-thinking");
  assert.equal(tokenDivisor, 2.0);
});
...
test("model-registry: handles base model without suffix", () => {
  ...
  const tokenDivisor = getModelTokenDivisor("qwen3.7-plus");
  assert.equal(tokenDivisor, 2.0);
});
