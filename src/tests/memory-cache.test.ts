...
test("MemoryCache: values above threshold are compressed", async () => {
  ...
  assert.ok(
    ...
    "Compression ratio should be greater than 1",
  );
...
test("MemoryCache: numeric-looking strings remain strings", async () => {
  const cache = new MemoryCache({ prefix: "test" });

  await cache.set("test:numeric-string" as CacheKey, "123");
  await cache.set("test:boolean-string" as CacheKey, "false");

  assert.equal(await cache.get("test:numeric-string" as CacheKey), "123");
  assert.equal(
    typeof (await cache.get("test:numeric-string" as CacheKey)),
    "string",
  );
  assert.equal(await cache.get("test:boolean-string" as CacheKey), "false");
  assert.equal(
    typeof (await cache.get("test:boolean-string" as CacheKey)),
    "string",
  );
...
test("MemoryCache: close clears cleanup interval", async () => {
  const cache = new MemoryCache({ prefix: "test" });

  assert.ok((cache as any).cleanupInterval);
  await cache.close();
